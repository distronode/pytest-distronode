...
def inject(start_path: Path) -> None:
    ...
    In the case of distronode > 2.9, initialize the collection finder with the collection path
    otherwise, inject the collection path into sys.path.

    :param start_path: The path where pytest was invoked
    """
    if not HAS_DISTRONODE:
        logger.error("distronode is not installed, plugin not activated")
        return
    if not HAS_YAML:
    ...
    collection_tree = ("collections", "distronode_collections", namespace, name)
    if start_path.parts[-4:] == collection_tree:
        ...
        name_dir = collections_dir / "distronode_collections" / namespace / name
    ...
    additional_collections_paths = [Path("~/.distronode/collections").expanduser()]
...
def inject_only() -> None:
    """Inject the current DISTRONODE_COLLECTIONS_PATH(S)."""
    envvar_name = determine_envvar()
...
def acf_inject(paths: list[str]) -> None:
    """Inject the collection path into the DistronodeCollectionFinder.

    :param paths: The paths to inject
    """
    # pylint: disable=protected-access
    if HAS_COLLECTION_FINDER:
        acf = _DistronodeCollectionFinder(paths=paths)
        acf._install()
...
def determine_envvar() -> str:
    """Use the existence of the DistronodeCollectionFinder to determine the distronode version.

    Distronode 2.9 did not have DistronodeCollectionFinder and did not support DISTRONODE_COLLECTIONS_PATH later versions do.

    :returns: The appropriate environment variable to use
    """
    if not HAS_COLLECTION_FINDER:
        return "DISTRONODE_COLLECTIONS_PATHS"
