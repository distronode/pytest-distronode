...
try:
    ...
    # if it does not exist because of old version of distronode, we don't need it
    co = None
...
POSITIVE_HOST_PATTERNS = [
    ...
    pytest.param("*[0-1]", 2, marks=pytest.mark.requires_distronode_v2(), id="9"),
    pytest.param(
        "*[0:1]",
        2,
        marks=pytest.mark.requires_distronode_v2(),
        id="10",
    ),  # this is confusing, but how host slicing works on v2
    pytest.param("*[0:]", 3, marks=pytest.mark.requires_distronode_v2(), id="11"),
]
...
POSITIVE_HOST_SLICES = [
    ...
    pytest.param(slice(0, 1), 2, marks=pytest.mark.requires_distronode_v2(), id="2"),
    pytest.param(slice(0, 2), 3, marks=pytest.mark.requires_distronode_v2(), id="4"),
...
def pytest_runtest_setup(item):
    # Conditionally skip tests that are pinned to a specific distronode version
    if isinstance(item, pytest.Function):
        # conditionally xfail
        mark = item.get_closest_marker("distronode_v2_xfail")
        if mark:
            item.add_marker(
                pytest.xfail(
                    reason="expected failure on >= distronode-2.*",
...
class PyTestOption:
    ...
    def __init__(self, config, testdir) -> None:
        ...
        self.inventory = testdir.makefile(
            ...
            [reachable]
            localhost distronode_connection=local distronode_python_interpreter='/usr/bin/env python'
            127.0.0.2 distronode_connection=local distronode_python_interpreter='/usr/bin/env python'
            127.0.0.3 distronode_connection=local distronode_python_interpreter='/usr/bin/env python'
            127.0.0.4 distronode_connection=local distronode_python_interpreter='/usr/bin/env python'
            127.0.0.5 distronode_connection=local distronode_python_interpreter='/usr/bin/env python'
...
def _clear_global_context():
    ...
    # if context object does not exist because of old version of distronode, we don't need it
    if co is not None:
