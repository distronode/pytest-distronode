...
def test_importerror_requires_v1():
    with pytest.raises(ImportError):
        ...
        import pytest_distronode.module_dispatcher.v1  # NOQA
...
def test_distronode_module_error(hosts):
    """Verify that DistronodeModuleError is raised when no such module exists."""
    from pytest_distronode.errors import DistronodeModuleError

    with pytest.raises(DistronodeModuleError) as exc_info:
