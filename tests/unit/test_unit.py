...
from pytest_distronode.units import inject, inject_only
...
def test_inject(
    ...
    monkeypatch.setattr(
        "pytest_distronode.units.get_collection_name",
        mock_get_collection_name,
    )

    (tmp_path / "collections" / "distronode_collections").mkdir(parents=True)

    inject(tmp_path)
    assert (
        tmp_path / "collections" / "distronode_collections" / "namespace" / "name"
    ).is_dir()
    assert (
...
def test_inject_only(
    ...
    """
    caplog.set_level(logging.DEBUG)
    monkeypatch.setenv("DISTRONODE_COLLECTIONS_PATH", str(tmp_path / "collections"))
...
def test_for_params():
    ...
    assert "--distronode-unit-inject-only" in proc.stdout.decode()
